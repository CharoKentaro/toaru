...
def show_tool(gemini_api_key, speech_api_key):
    ...
    # session_stateによる、純粋な、状態管理 (変更なし)
    ...
    # ★★★【叡智の最終形態】★★★
    # もはや、世界を、入れ替えない。ただ、戒厳令を、敷くだけである。
    if is_limit_reached:
        # --- 戒厳令下の、布告 ---
    ...
    st.caption(f"🚀 あと {max(0, usage_limit - st.session_state.translator_usage_count)} 回、翻訳できます")
    
    col1, col2 = st.columns([1, 2])
    with col1:
        # ★★★ ここに、戒厳令を、適用する ★★★
        audio_info = mic_recorder(start_prompt="🎤 話し始める", stop_prompt="⏹️ 翻訳する", key='translator_mic', disabled=is_limit_reached)
    with col2:
        # ★★★ ここに、戒厳令を、適用する ★★★
        text_prompt = st.text_input("または、ここに日本語を入力してEnterキーを押してください...", key="translator_text", disabled=is_limit_reached)
