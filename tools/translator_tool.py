...
# ===============================================================
# 補助関数 (私たちが最後に成功を確認した、シンプルで強固なバージョン)
# ===============================================================
def translate_with_gemini(content_to_process, api_key):
    ...
    # --- エラー処理を、最も安定的だった、この形に戻します ---
    except Exception as e:
        st.error(f"AI処理エラーが発生しました: {e}")
        return None, None

# ===============================================================
# 専門家のメインの仕事 (私たちの叡智の集大成に、最高の「配慮」を添えて)
# ===============================================================
def show_tool(gemini_api_key):
    ...
    if is_limit_reached:
        ...
        with st.expander("💡 このツールのAIについて"):
            ...
            **もし、エラーメッセージが表示されましたら：**
            
            APIキーの利用上限に達した可能性があります。少し時間をあけるか、明日以降に再試行してください。
